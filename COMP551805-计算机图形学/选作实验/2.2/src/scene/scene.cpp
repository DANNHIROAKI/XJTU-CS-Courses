...
void Scene::simulation_update()
{
    ...
    float elapsed = duration_cast<duration>(elapsed_time).count();
    float elapsed_seconds=elapsed;

    for (; elapsed_seconds > time_step; elapsed_seconds -= time_step) {
        auto it = all_objects.begin();
        while (it != all_objects.end()) 
        {
            // 使用 Object::update 方法更新物体状态
            (*it)->update(all_objects);
            ++it;
        }
    }

    auto remain=std::chrono::duration<float>(elapsed_seconds);
    duration remaining_time = remain;
    auto t=elapsed_time - remaining_time;
    auto update=std::chrono::duration_cast<std::chrono::steady_clock::duration>(t);
    last_update = last_update + update;
}
//////////////////////////////////////////////////////////