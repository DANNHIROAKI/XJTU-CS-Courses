...
void Object::update(vector<Object*>& all_objects)
{
    ...
    auto obj_iter = all_objects.begin();
    while (obj_iter != all_objects.end()) {
        auto object = *obj_iter;
        (void)object; // 如果不需要使用 object 变量，这行可以省略

        size_t i = 0;
        while (i < mesh.edges.count()) {
            array<size_t, 2> v_indices = mesh.edge(i);
            (void)v_indices; // 如果不需要使用 v_indices 变量，这行可以省略
            if (BVH_for_collision) {
                // BVH_for_collision 的处理逻辑
            } else {
                // 非 BVH_for_collision 的处理逻辑
            }
            ++i;
        }
