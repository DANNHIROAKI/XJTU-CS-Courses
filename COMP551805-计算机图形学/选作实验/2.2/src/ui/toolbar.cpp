    ...
    if (ImGui::BeginTabItem("Layout")) {
        ...
        if (selected_object != nullptr) {
            ...
            */
            ...
            // 计算角度差并转换为弧度
            float delta_x_radians = (x_angle - original_x_angle) * M_PI / 180.0f;
            float delta_y_radians = (y_angle - original_y_angle) * M_PI / 180.0f;
            float delta_z_radians = (z_angle - original_z_angle) * M_PI / 180.0f;

            // 创建各轴上的旋转表示
            AngleAxisf rotation_x(delta_x_radians, Vector3f::UnitX());
            AngleAxisf rotation_y(delta_y_radians, Vector3f::UnitY());
            AngleAxisf rotation_z(delta_z_radians, Vector3f::UnitZ());

            // 根据角度旋转构造四元数
            Quaternionf qx(rotation_x);
            Quaternionf qy(rotation_y);
            Quaternionf qz(rotation_z);
