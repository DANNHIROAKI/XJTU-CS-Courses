...
> > > >
> > > > :two:核心问题：条件的$A$与证据的$A'$不一定完全相同，例如
> > > >
> > > > ```txt
...
> > >
> > > :two:搜索过程：给定初始/最终结点，找到最佳路径，==详见$A*$算法==
> >
> > ### 5.2.2. 广度优先搜索
> >
> > > :one:概述
...
> > > | 复合指标        | $\text{precision}(T) = \frac{|a|}{|a + b|} $，$\text{recall}(T) = \frac{|a|}{|a + d|} $，$\text{Accuracy}(T) = \frac{|a + c|}{|a + b + c + d|}$ | 见下图                                                       |
...
> > > 3. 每个单元都有输出
> > >
> > > :two:单元之间：PDP有很多单元，按照一定模式连接，系统通过经验修改连接强度学习规则
> > >
> > > :three:原理：通过连接强度的变化来产生学习
>
> ## 7.2. 神经网络拓扑形态
>
> > :one:前馈网络：每层直接收上一层输入，如BP
> >
...
> ## 7.3. 神经网络的学习
>
> > :one:学习概念：改变神经网络内部表示，使得输入输出向好的方向发展
> >
> > :two:学习分类：
> >
...
> > > <img src="https://raw.githubusercontent.com/DANNHIROAKI/New-Picture-Bed/main/img/image-20231230001544907.png" alt="image-20231230001544907" style="zoom:50%;" /> 
> > >
> > > :one:输入：所有输入信号和，$\text{net}_j = \sum\limits_{i=1}^{n} w_{ij}x_i = \sum\limits_{i=1}^{n} w_{ji}y_i$
> > >
> > > :two:输出：可以是$ y_j = f(\text{net}_j) = \cfrac{1}{1 + e^{-\text{net}_j}}$，也可以是$\text{tanh}$，必须是处处可微分的
> > >
> > > :two:拓扑结构：由输入维度，隐藏层数量，输出维度决定
> >
> > ### 7.4.2. 算法过程
> >
> > > :one:初始化：设置权值为小随机数
> > >
> > > :two:向前传播：输入一个样本$X_p$，输出$Y_p$
> > >
> > > :three:向后传播：
> > >
...
> >
> > > :one:上一层第$i$个神经元$\xleftrightarrow{连接}$输出层第$j$个神经元
> > >
...
> > >
> > > :two:$k-1$层第$i$个神经元$\xleftrightarrow{连接}$$k$层第$j$个神经元：
> > >
...
> > ### 7.4.4. BP算法注意事项
> >
> > > :one:激活函数：满足非线性/有界/连续/可微 
> > >
> > > :two:目标向量的选择：向量的每个分量，不超过激活函数的值域
> > >
> > > :three:权系数的初始化：随机化均匀赋值
> > >
> > > :four:学习速率$\eta$：一般取0.1，过大会导致不收敛。过小会收敛慢
> > >
> > > :five:误差函数局部极小值：当训练过程被困在局部极小值时，需要对权值重新初始化，然后重新训练
> > >
> > > :six:学习曲线：
> > >
...
> > >
> > > :seven:过拟合：对训练集分类能力强，对测试集却很差，solution是在确认集平均误差函数最小时终止
> > >
> > > :eight:隐藏层数目：过多容易过拟合，一般取样本数/10
...
> >
> > > :one:含义：对观察到的物理对象识别分类
> > >
> > > :two:模式：存储于计算机内的有关物理对象的观测信息
> > >
> > > :three:如何让机器模式识别：采集数据+模式识别算法
> > >
> > > :four:模式识别的意义：对外界感知和识别是智能的基础
> >
> > ### PS.1.2. 模式识别系统设计：鱼类分类为例
> >
> > > :one:获取观测量：比如获得+分割图像，得到单个鱼的图像
> > >
> > > :two:特征提取：
> > >
...
> > >
> > > :three:分类器训练：根据样本集，构造一个判定函数$d(x)$来实现分类
> > >
...
> > >
> > > :four:分类器测试：用新样本测试分类器性能，看能不能正确分类得到误差，随后进行优化
> > >
> > > :five:分类器设计的考虑因素：
> > >
> > > 1. 两类鱼特征向量的真实分布
